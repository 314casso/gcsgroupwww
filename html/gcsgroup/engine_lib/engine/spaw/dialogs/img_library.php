...
<tr>
<input type=hidden value="0" name="ord">
<script>
function send_url(varr){
	var thisForm = document.libbrowser; 
	var var_srt_hidden = '';
	if (varr) document.getElementById("ord").value = varr; 
	document.libbrowser.submit();
}
</script>
  <td valign="top" align="left"><b><?php echo $l->m('images')?>:</b><br><small>Сортировка: 
<a href="javascript:send_url('<? if ($_POST[ord] != 1) echo "1";?>')">имя<? if ($_POST[ord]==1 || !$_POST[ord]) { if ($_POST[ord] != 1) echo "+"; else echo "-";}?></a> | <a href="javascript:send_url('<? if ($_POST[ord] != 2) echo "2"; else echo "3";?>')">дата<? if ($_POST[ord]==2 || $_POST[ord]==3) { if ($_POST[ord] != 2) echo "-"; else echo "+";}?></a>
</small></td>
</tr>
<tr>
  <td valign="top" align="left">
...
/*    onchange="if (this.selectedIndex &gt;=0) document.getElementById('imgpreview').location = '<?php echo $spaw_base_url.$imglib?>' + this.options[this.selectedIndex].value;" ondblclick="selectClick();"> */
    ...
    { 
      ...
      while (false !== ($entry = $d->read())) {
        ...
        {
		$file_arr[] = array (NAME =>$entry, TIME =>filemtime($_root.$imglib.$entry));
          ?>
      ...
      $d->close();
	if ($ord == 1)
		usort($file_arr, cmp_name_low);
	elseif ($ord == 2)
		usort($file_arr, cmp_time);	
	elseif ($ord == 3)
		usort($file_arr, cmp_time_low);	
	else
		usort($file_arr, cmp_name);
	for ($i=0;$i < count($file_arr);$i++){
		?>
          <option value="<?php echo $file_arr[$i][NAME];?>" <?php echo ($file_arr[$i][NAME] == $img)?'selected':''?>><?php echo $file_arr[$i][NAME]?></option>
		<?
	}
...
function cmp_name_low($a, $b) {
	return ($a[NAME] < $b[NAME]) ? 1 : -1; 
}
function cmp_time($a, $b) {
	return ($a[TIME] < $b[TIME]) ? 1 : -1; 
}
function cmp_time_low($a, $b) {
	return ($a[TIME] < $b[TIME]) ? -1 : 1; 
}
?>

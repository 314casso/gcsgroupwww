...
<?
	...
	$sql="SELECT 
			(max( x ) + 1) cols,
			(max( y ) + 1) rows
		 from engine_TABLEDOC_cells where doc_id=$parent_doc[id]";
...
//	if ($page/$cols>$rows ||$page%$parent_doc_spec[rowsperpage])
	...
	$sql="select * from engine_TABLEDOC_cells where doc_id=$parent_doc[id] order by  y,x $limit";
...
//									alert (document.images.cellimg.src);
									...
									if (file_exists("$engine_show_root/engine/pics/TABLEDOC/pic{$parent_doc[id]}_".$cells[$x][$y][ids].".gif")){
										$table.="$engine_show_htroot/engine/pics/TABLEDOC/pic{$parent_doc[id]}_".$cells[$x][$y][ids].".gif";
										}
									else if (file_exists("$engine_show_root/engine/pics/TABLEDOC/pic{$parent_doc[id]}_".$cells[$x][$y][ids].".jpg")){
										$table.="$engine_show_htroot/engine/pics/TABLEDOC/pic{$parent_doc[id]}_".$cells[$x][$y][ids].".jpg";
										}
									else
										$table.="$engine_show_htroot/engine/pics/TABLEDOC/0.gif";
				$table.=				
									"';}\" onkeyup=\"javascript:{

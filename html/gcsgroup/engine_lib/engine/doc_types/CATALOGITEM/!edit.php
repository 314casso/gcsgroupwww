...
/*
 *  $Id: edit.php,v 1.8 2005/01/19 19:08:45 dave Exp $
 *
 *  $Log: edit.php,v $
	...
	if (!$f[select]) {
		...
		if (!$f[multiselect]) {
			...
			$param .= "<tr><td class=top>$f[name]</td><td>
				<select name=val$f1[id]_forparam$f[id] style='width=400;'>";
			...
			while ($f1 = mysql_fetch_array($r1)) {
				...
				if ($f[type] == "Изображение") {
					...
					$param .= "<tr><td valign=top alihn=right>$f1[val]</td><td>$pic</td><td align=right>";
					if (!$f2[id])

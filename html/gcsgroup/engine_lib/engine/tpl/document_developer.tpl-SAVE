...
function ShowBookmark(book_id)
{
        table = document.getElementById('table_'+bookmark_id);
        table.style.display = 'none';
        td    = document.getElementById('td_'+bookmark_id);
        td.className = 'td_bookmark2_off';

        table = document.getElementById('table_'+book_id);
        table.style.display = '';
        td    = document.getElementById('td_'+book_id);
        td.className = 'td_bookmark2_on';

        bookmark_id = book_id;
}

function TableHide(table_id, table_hide)
{	table = document.getElementById('table_'+table_id);
	if (table)
	{		table.style.display = (table_hide == 1) ? 'none' : '';
	}
}
</script>
  ...
  <tr>
    <td>
      <table cellspacing="1" cellpadding="3" border="0" class="table_bookmark">
        <tr><td class="td_bookmark_on" align="center" id="td_spec" nowrap="nowrap">&nbsp;<a href="javascript:ShowTable('spec'); TableHide('extended', 0);" class="doc_menu">{DOC_TYPE}</a>&nbsp;</td></tr>
      </table>
    </td>
    <td>
      <table cellspacing="1" cellpadding="3" border="0" class="table_bookmark">
        <tr><td class="td_bookmark_off" align="center" id="td_doc">&nbsp;<a href="javascript:ShowTable('doc'); TableHide('extended', 1);" class="doc_menu">Документ</a>&nbsp;</td></tr>
      </table>
    </td>
    <td>
      <table cellspacing="1" cellpadding="3" border="0" class="table_bookmark">
        <tr><td class="td_bookmark_off" align="center" id="td_html">&nbsp;<a href="javascript:ShowTable('html'); TableHide('extended', 1);" class="doc_menu">HTML</a>&nbsp;</td></tr>
      </table>
    </td>
    <td>
      <table cellspacing="1" cellpadding="3" border="0" class="table_bookmark">
        <tr><td class="td_bookmark_off" align="center" id="td_rubrique">&nbsp;<a href="javascript:ShowTable('rubrique'); TableHide('extended', 1);" class="doc_menu">Рубрики</a>&nbsp;</td></tr>
      </table>
    </td>
  </tr>
</table>
<FORM name="editor" method="post" action="?action=showdoc&do=1" onSubmit='{SUBMIT}' enctype="multipart/form-data" style="margin: 0px; padding: 0px;">

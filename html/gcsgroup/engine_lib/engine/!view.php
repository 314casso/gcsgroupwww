  ...
  <tr>
    <td width="100%" valign="middle" align=center >
      <table width="350" cellspacing="1" cellpadding="3" class="line">
        ...
        <tr><td align="center" style="height: 60px;">Документ не доступен, так как пользователь <b><?=$ologin?></b> редактирует его с <b><?=date('H:i',strtotime($lastopen))?></b>.</td></tr>
      </table>
    </td>
  </tr>
</table>
...
if ($action == 'showgroup')
{
        if ($usrid)
        {
                $sql = "select * from engine_users where         id =$usrid";
                $usr = mysql_query("$sql") or die (mysql_error());
                $usr = mysql_fetch_array($usr);
                $ltr = substr($usr[login], 0, 1);
                $id = $usr[ugroup];
        }
        ...
        if ($userid and $userid != -1) {
            ...
            $sql .= "           name='$username',
                        ...
                        $sql = "delete from  engine_users  where id=$userid        ";
                $DBResult = mysql_query("$sql") or die (mysql_error());
                if (!$del)
                        echo "
                                <script>
                                                document.location='view.php?action=showgroup&usrid=$userid&id=$groupid';
                                </script>
                                ";
                else
                        echo "
                                <script>
                                                document.location='view.php?action=showgroup&id=$groupid';
                                </script>
                                ";
                exit;
        }
        if ($userid) {
                if (mysql_num_rows($DBResult = mysql_query("select * from engine_users where login='userlogin'"))) {
                        echo "<font color=#FF0000>Такой пользователь уже есть!</font>";
                } else {
                        if ($active)$active = 1;
                        else $active = 0;
                        $sql = "insert into engine_users(active,role,login,pass,name,mname,lname,phone,firm,email,info,`ugroup`) values('$active','$role','$userlogin','".md5(trim($userpass))."','$username','$usermname','$userlname','$userphone','$userfirm','$useremail','$userinfo','$groupid')";

                        $DBResult = mysql_query("$sql") or die (mysql_error());
                        echo "
                                <script>
                                                document.location='view.php?action=showgroup&usrid=" . mysql_insert_id() . "';
                                </script>
                                ";
                }
                exit;
        ...
        $sql = "select * from engine_users where `ugroup`=$id order by login";
        $usrs = mysql_query("$sql") or die ($sql . "<br><br>" . mysql_error());
        while ($usr = mysql_fetch_array($usrs)) {
                if ($letter != substr($usr[login], 0, 1)) {
                        $letter = substr($usr[login], 0, 1);
                        if ($ltr != $letter) {
                                $ltrs .= "<a class=link href=view.php?action=showgroup&ltr=$letter&id=$id>_$letter</a><br>";
                        } else {
                                $ltrs .= "<a class=link href=view.php?action=showgroup&ltr=$letter&id=$id><strong>_$letter</strong></a><br>";
                        }
                }
        }
        $tpl->assign(
        ...
        if ($ltr) {
                $sql = "select * from engine_users where  `ugroup`=$id and  login like '$ltr%'  order by login";
                $usrs = mysql_query("$sql") or die ($sql . "<br><br>" . mysql_error());
                while ($usr = mysql_fetch_array($usrs)) {
                        $usr[active]?$uimg = "user.gif":$uimg = "user_gray.gif";
                        $usrlist .= "<tr><td><img src='img/$uimg' border=0></td><td><a class=link href=view.php?action=showgroup&usrid=$usr[id]>$usr[login]</a></td></tr>";
                }
                $tpl->assign(
                        array(
                                USERLIST => $usrlist
        ...
        if ($usrid) {
                ...
                $r = mysql_query($sql) or die("SQL error " . mysql_error() . " in: $sql\n");
                while ($f = mysql_fetch_array($r)) {
                        if ($usr[ugroup] == $f[id]) $moveto .= "<option selected value=$f[id]>$f[name]</option>";
                        else $moveto .= "<option value=$f[id]>$f[name]</option>";
                }
                $moveto = "<td><strong>Переместить в группу</strong></td><td colspan=2><select name=group>$moveto</select></td>";
                ...
                $tpl->assign(
                        array(
                                ...
                                GROUPID => $usr[ugroup],
                                USERLOGIN => $usr[login],
                                USERPASS => (strlen($usr[pass]) == 32 || strlen($usr[pass]) == 20) ? '*****' : $usr[pass],
                                USERNAME => $usr[name],
                                USERMNAME => $usr[mname],
                                USERLNAME => $usr[lname],
                                USERPHONE => $usr[phone],
